...
class NEBEAE:
    ...
    def evaluate(self, rho=0.1, Lambda=0):
        ...
        while (Jp-J)/Jp >= self.epsilon and ITER < self.maxiter and self.oae == 0 and self.numerr == 0:
            t_A=self.abun_par(self.Y, self.Ynorm)
            a_Time += t_A[0]
            t_D,self.Dm= self.prob_par(self.Y, self.Ynorm)
            d_Time += t_D
            Pp=self.P.copy()
            if self.numerr == 0:
                t_P=self.endmember_par(self.Y,self.Ynorm,Pp)
                p_Time += t_P[0]
            Jp = J
            a=np.multiply(np.tile((1-self.Dm).T,[self.Mbandas,1]),self.P@self.A)
        ...
        t_D,self.Dm = self.prob_par(self.Z,self.Z)
